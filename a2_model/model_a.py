...
def model(jsdatanum):
    ...
    if os.path.exists('a2_o_base_str_txt'):
        shutil.rmtree('a2_o_base_str_txt')
    if os.path.exists('a2_o_wobfuscator'):    
        shutil.rmtree('a2_o_wobfuscator')
    if os.path.exists('a2_o_wasm'):
        shutil.rmtree('a2_o_wasm')
    if os.path.exists('a2_o_wat'):
        shutil.rmtree('a2_o_wat')
    if os.path.isfile('a2_model/error_file_list.txt'):
        os.remove('a2_model/error_file_list.txt')

    os.mkdir('a2_o_base_str_txt')
    os.mkdir('a2_o_wobfuscator')
    os.mkdir('a2_o_wasm')
    os.mkdir('a2_o_wat')
    ...
    for num1 in range(jsdatanum):
        jspath="a1_o_jsdata/"+str(num1+1)+".js"
        wobfuscator_o_path="a2_o_wobfuscator/"+str(num1+1)
        ...
        try:
            ...
            try:
                ...
                choise_base_path="a2_o_base_str_txt/"+str(num1+1)+".txt"
                try:
                ...
                f3=open("a2_model/error_file_list.txt",'a')
                f3.write(str(num1+1)+"\n")
                f3.close()
                continue
        except FileNotFoundError as e:
        ...
        wasm_a_o_path="a2_o_wasm/"+str(num1+1)+".wasm"
        ...
        wat_a_o_path="a2_o_wasm/"+str(num1+1)+".wat"
        ...
        sp4=subprocess.run(['mv', wat_a_o_path, 'a2_o_wat'], stdout=subprocess.PIPE, text=True)
        if sp4.returncode!=0:
